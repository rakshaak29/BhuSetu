...
async function runTests() {
  ...
  const lookupResult = await repository.getEvidenceByHash(tamperedSha);
  assert.ok(!lookupResult, 'Tampered file hash must return null/falsy (producing MISMATCH state)');
  ...
  await repository.saveEvidenceEvent(newPendingEvent);
  ...
  assert.ok(history.length >= 1, 'Fabric ledger block chain must contain history');
  ...
  const pcl042 = await repository.getParcelById('PCL-AP-GNT-042');
